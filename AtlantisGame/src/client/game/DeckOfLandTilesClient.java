...
public class DeckOfLandTilesClient implements Serializable {

	private ArrayList<LandTileClient> deckOfTiles = new ArrayList<>();
	// i put K as class variable for it not to reset with the loop
	int k = 1;
	// random object to remove one land tile from each color 
	Random r = new Random();

	public DeckOfLandTilesClient() {
		shuffleTiles();
	}
	...
	public LandTileClient dealTile() {
		return (deckOfTiles.size() > 0) ? deckOfTiles.remove(deckOfTiles.size() - 1) : null; // first semester
	}
	public LandTileClient getTileDetails(int i){
		return deckOfTiles.get(i);
	}

	public ArrayList<LandTileClient> getDeckOfTiles() {
		return deckOfTiles;
	}



	public void setDeckOfTiles(ArrayList<LandTileClient> deckOfTiles) {
